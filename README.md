...
> **Phase 22 (2026-05-09)**: **Growth-Agent Migration P0 + P1 fix + Memory v4.2** — Nous Hermes Agent 기준 5-loop closed-loop wiring. 8 generation artifacts + 14 wiring scripts + 10 marker block + 1 yaml block + W3 timer 5종 + 1 memory schema migration. v4.2 추가: import pipeline (Claude/ChatGPT/Discord 자동 메모리 → `data/processed_memory/`) + sanitized-only writer + `compile_split_memory` (USER.md / MEMORY.md split) + P2 retrieval supplement (default OFF). 운영 상태와 D+7/D+14/D+30 J-checkpoint 절차는 [docs/growth_agent_ops.md](docs/growth_agent_ops.md) 참조.
>
...
## Hermes Growth-Agent Status (Phase 22, Day-0)

5-loop closed-loop 운영 상태. 자세한 절차/체크리스트는 [docs/growth_agent_ops.md](docs/growth_agent_ops.md).

| Loop | 상태 | 주요 신호 |
|---|---|---|
| 1 Memory | **Active Growing** | **Layer 1 (legacy A/B, default ON)**: `data/state.db` `memos` 테이블에 `source='generated_candidates'` 28 rows + `user_feedback_style` 5 rows + Claude import results. 봇 `_maybe_inject_memory()` Layer 1 (top-k=3) recall 코퍼스. <br>**Layer 2 (P2 retrieval supplement, default OFF)**: `data/processed_memory/*.md` + `data/memory/{USER,MEMORY}.md` (compile_split_memory 결과). `memory_retrieval_enabled=true` 시점에 `MemoryInjectionService`가 retriever_v1 ab key로 별개 inject. <br>compile 결과는 `_compose_prompt`의 `read_prompt_prepend()`가 master prompt에 항상 prepend (Layer 2 flag와 독립) |
| 2 Skill (creation) | **Active for draft creation** | 17 → 27 SKILL.md (P0c.4가 10개 promoted). W9/W10/W11 draft 경로 모두 wiring 완료. auto-install은 P1 toggle OFF 유지 → draft/PR flow |
| 3 User Model | **Active Growing** | `profiles/SOUL.generated.md` W4 marker로 master prompt 매 호출 inject. W8 dialectic 다음 월요일 06:00 KST 첫 실행 |
| 4 Cron Reflection | **Bootstrap-Ready** | W7 SelfReview 다음 일요일 21:00 KST 첫 실행. W11 marker block (`curator_job` + `skill_promoter`) self-review artifact 소비 준비 |
| 5 Delegation | **Observation-Only (P0)** | W12 `bot_stdout.log`에 `w12.delegation_suggestion` 만 기록. **active dispatch biasing은 P2** (`task.suggested_handles` 추가 후 dispatch hookup 필요) |

**Cron timers**: 9/9 active (Windows schtasks). 일요일 21:00 SelfReview → 22:00 Reflection → 22:30 ABReport → 23:00 Curator → 23:30 SkillPromoter, 토요일 23:00 SkillSelfModify, 월요일 06:00 Dialectic / 12:00 DelegationPattern, 30분 단위 SkillDraftQueueDrainer.

**P1 toggles — 모두 OFF 유지** (D+7~D+14 데이터 누적 후 재검토):
- `skill_promoter_auto_install=false` (constructor default, `src/jobs/skill_promoter.py:86`)
- `skill_hot_reload_enabled=false`
- `feedback_keyword_match_enabled=false`
- `memory_retrieval_enabled=false` (`src/config.py:225`) — Layer 2 P2 retrieval supplement gating. `data/processed_memory/`에 entry 누적 + USER.md/MEMORY.md compile 완료 후 검토. `memory_retrieval_v1` ab key는 Phase 21 `memory_inject` A/B와 독립

**Memory store 구조 (post-v4.2)**:
- **Layer 1 recall** — `data/state.db` `memos` 테이블 (`src/gateway/discord_bot.py:64` `SqliteMemory(settings.state_db_path)`). LIKE substring search, top-k=3, default ON.
- **Layer 2 retrieval supplement** — `data/processed_memory/*.md` (sanitized derivatives) + `data/source_manifests/*.jsonl` (manifest-only, raw payload 부재). `memory_retrieval_enabled=true` 시점에 `MemoryInjectionService`가 retriever로 검색.
- **Compiled prompt prepend** — `data/memory/USER.md` + `data/memory/MEMORY.md` (`compile_split_memory` 결과). `_compose_prompt`의 `read_prompt_prepend()`가 항상 prepend (Layer 2 flag와 독립).
- **Raw 원문** — `data/external_memory/snapshots/` (gitignored, never tracked).
- **별도 store**: 기존 `data/memory/MEMORY.md` 위치는 v4.2에서 compile_split_memory 결과로 사용 중 (CuratorJob 큐레이트 결과는 `MemoryInjectionService` Layer 2 입력으로도 활용).

**Emergency disable**:
- `setx HERMES_DISABLE_GROWTH_BLOCKS true` → W4/W6/W10/W11×2/W12 marker block 즉시 short-circuit, P0 이전 동작 복원
- `setx HERMES_MEMORY_RETRIEVAL_ENABLED false` → Layer 2 retrieval supplement 즉시 OFF (W marker block과 독립). 기본값이 false라 평소엔 환경변수 미설정 상태로 충분
- 봇 재시작 후 effective
