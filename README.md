...
> **Phase 22 (2026-05-09)**: **Growth-Agent Migration P0 + P1 fix** — Nous Hermes Agent 기준 5-loop closed-loop wiring. 8 generation artifacts + 14 wiring scripts + 10 marker block + 1 yaml block + W3 timer 5종 + 1 memory schema migration. 운영 상태와 D+7/D+14/D+30 J-checkpoint 절차는 [docs/growth_agent_ops.md](docs/growth_agent_ops.md) 참조.
>
...
| Loop | 상태 | 주요 신호 |
|---|---|---|
| 1 Memory | **Active Growing** | `data/state.db` (settings.state_db_path) `memos` 테이블에 `source='generated_candidates'` 28 rows + `user_feedback_style` 5 rows. 봇 `_maybe_inject_memory()` (default ON, top-k=3) recall 코퍼스 |
...
**Memory DB path 주의**: 봇 SqliteMemory가 사용하는 recall 코퍼스는 `data/state.db`의 `memos` 테이블 (`src/gateway/discord_bot.py:64` `SqliteMemory(settings.state_db_path)`). `data/memory/MEMORY.md`는 CuratorJob이 별도로 큐레이트하는 사람 review용 store이며 recall 코퍼스가 아니다.

**Emergency disable**: `setx HERMES_DISABLE_GROWTH_BLOCKS true` (User-level env) 후 봇 재시작 → W4/W6/W10/W11×2/W12 marker block 모두 즉시 short-circuit, P0 이전 동작 복원.
