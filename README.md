...
| **Skills** — 반복 작업 절차화 | 🟡 부분 | profile-level `SKILL.md` 10개 (`profiles/*/skills/**`) + slash skill 5개 (`HybridMemoSkill`/`HybridStatusSkill`/`HybridBudgetSkill`/`CalendarSkill`/`KanbanSkill`). `SkillLibrary` 가 인덱싱. Curator 가 promotion / archive 후보 markdown 자동 surface. **자동 승격 자체는 사람 review 단계** (Phase 7 = 6 카테고리 / 17 agent 통합 예정). |
...
| **Delegation** — sub-agent 병렬 | 🟡 인터페이스 | `Delegator` Protocol + `SequentialHermesDelegator` (Phase 5b 에서 진짜 병렬 — 사용자 제시 6 카테고리 / 17 agent 체계 위에서). |
...
- [ ] **Phase 7 (다음)** — 6 카테고리 / 17 agent 통합 (RESEARCH / PLANNING / IMPLEMENTATION / QUALITY / DOCUMENTATION / INFRASTRUCTURE) — `@coder`, `@reviewer` 등 sub-agent 인터페이스
- [ ] **Phase 8** — Phase 5b 진짜 병렬 sub-agent delegation
- [ ] **Phase 9** — Slack gateway, Discord 슬래시 명령 확장 (`/kanban` 외)
