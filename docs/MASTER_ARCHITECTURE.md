# Master Architecture (Phase 10, 2026-05-06)
...
> ``OpenCodeAgentDelegator`` 가 각 ``@handle`` 별로 독립 opencode 호출
> 을 ``asyncio.gather`` 로 동시 실행 후 결과 집계. Pre-Phase-8 reference
...
- **흐름**:
  ...
  7. `_dispatch_master` → opencode.run → `Critic.evaluate`
  8. `_finalize` → `ExperienceLogger.append` (`model_provider="opencode"`, `agent_handles=[...]`)
...
>= 2` 이면, master 가 **단일 호출 대신** [`OpenCodeAgentDelegator`](../src/core/delegation.py)
로 라우팅:
...
@coder + @reviewer + @tester
    ...
    sem = Semaphore(master_parallel_max_concurrency)
        ↓ 동시 opencode subprocess 호출 (각각 자체 SKILL.md system prompt)
...
| 출력 | 트리거 | 위치 |
|---|---|---|
| **ExperienceLog** | 모든 task 종료 (master + opencode session import) | `logs/experience/{YYYY-MM-DD}.jsonl` |
...
- `hitl_*` / `watcher_*` / `opencode_enabled` / `job_factory_*` / `use_new_job_factory`
...
- ✅ **Phase 10 (2026-05-06)** — `OpenCodeAgentDelegator.delegate_many` 진짜
