...
- **흐름**:
  ...
  2. `IntentRouter.route` → IntentResult (RuleLayer / 슬래시 skill / fallthrough)
...
- **Phase 9** — Master 가 IntentRouter 결과의 `@coder` 같은 mention 인식
  → AgentRegistry.by_handle → agent SKILL.md 를 system prompt 에 inject.
- **Phase 10** — `Delegator.delegate_many` 의 진짜 병렬 실행 + agent 간
  결과 집계.
