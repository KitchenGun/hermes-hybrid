...
description: Capture learnings from the current session into the project-rules file (AGENTS.md, AGENTS.md, or local override) so future sessions benefit. Use when the user says "revise the rules", "update AGENTS.md / AGENTS.md with what we just learned", "save this to project memory", "remember this for next time", or at the end of a productive session when valuable context has emerged that is not yet documented. This is the COMPLEMENT to agents-md-improver: improver audits, this one captures.
license: MIT (translated from anthropics/Codex-plugins-official/plugins/Codex-md-management/commands/revise-Codex-md.md)
---

# AGENTS.md / AGENTS.md Revise
...
OpenCode reads project rules from `AGENTS.md` (native) or `AGENTS.md` (Codex compatibility). When this skill creates a new file, prefer `AGENTS.md`. When updating an existing file, edit whichever the project already uses.
...
```bash
find . \( -name "AGENTS.md" -o -name "AGENTS.md" -o -name ".agents.local.md" -o -name ".Codex.local.md" \) 2>/dev/null | head -20
```

Decide where each addition belongs:

- **`AGENTS.md` / `AGENTS.md`** — team-shared, committed to git. Use for project-wide conventions, build commands, architectural notes.
- **`.agents.local.md` / `.Codex.local.md`** — personal / local only, gitignored. Use for personal preferences that should not affect teammates.
...
- Do not include sensitive information (API keys, tokens, internal URLs) in committed rules files. Put those in `.agents.local.md` / `.Codex.local.md` or in environment variables.
