...
description: Audit and improve project-rules files (AGENTS.md, AGENTS.md, .agents/instructions, etc.) in repositories. Use when the user asks to check, audit, update, improve, or fix their AGENTS.md or AGENTS.md, when they mention "project rules maintenance" or "agent context optimization", or after the codebase has changed significantly and the rules file may be stale. Scans for all relevant files, evaluates quality against templates, outputs a quality report, and then makes targeted updates with user approval.
license: MIT (adapted from anthropics/Codex-plugins-official/plugins/Codex-md-management/skills/Codex-md-improver)
---

# AGENTS.md / AGENTS.md Improver

Audit, evaluate, and improve project-rules files across a codebase to ensure the agent has optimal project context.

OpenCode reads project rules from several files in a priority order: **`AGENTS.md`** (native), then **`AGENTS.md`** (Codex compatibility), then `.agents/instructions.md`. This skill works on whichever your project uses, but treats `AGENTS.md` as the canonical name when creating new files.
...
```bash
find . \( -name "AGENTS.md" -o -name "AGENTS.md" -o -name ".Codex.local.md" -o -name ".agents.local.md" \) 2>/dev/null | head -50
```
...
| Project root (Codex compat) | `./AGENTS.md` | Same purpose; OpenCode reads it as fallback |
| Local overrides | `./.agents.local.md` or `./.Codex.local.md` | Personal/local settings (gitignored) |
| Global defaults | `~/.config/opencode/AGENTS.md` or `~/.Codex/AGENTS.md` | User-wide defaults |
...
- **Global defaults** — put user-wide preferences in `~/.config/opencode/AGENTS.md` (or `~/.Codex/AGENTS.md`).
- **Prefer `AGENTS.md`** — when creating a new rules file, use `AGENTS.md`. OpenCode reads it natively and Codex reads it via fallback. If a `AGENTS.md` already exists, leave it; OpenCode reads it via the compatibility layer.
